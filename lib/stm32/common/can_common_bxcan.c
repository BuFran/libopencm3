		 ...
		 uint8_t *length)
{
	...
	addr->ext = (CAN_RIxR(canport, fifo_id) & CAN_RIxR_IDE) != 0;
	/* Get standard CAN ID. */
	addr->cobid = (CAN_RIxR(canport, fifo_id) & CAN_RIxR_STID) >>
			CAN_RIxR_STID_SHIFT;
	/* Get extended CAN ID, copy only extension bits */
	addr->extid = (CAN_RIxR(canport, fifo_id) &
